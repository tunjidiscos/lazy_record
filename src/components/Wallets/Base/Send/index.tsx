...
const BaseSend = () => {
  ...
  const getBase = async () => {
    try {
      ...
      if (response.result) {
        ...
        await getBaseNonce(response.data.address);
      }
    } catch (e) {
  ...
  const getBaseGasLimit = async (from: string): Promise<boolean> => {
    try {
  ...
  const getBaseMaxPriortyFee = async () => {
    try {
  ...
  const getBaseNonce = async (address: string) => {
    if (address && address != '') {
      try {
  ...
  const checkGasLimit = async (): Promise<boolean> => {
    ...
    return await getBaseGasLimit(fromAddress);
  };
          ...
          <>
            <Box textAlign={'center'}>
              <Stack direction={'row'} alignItems={'center'} justifyContent={'center'} mt={4}>
                <Image src={BaseSVG} alt="" width={25} height={25} />
                <Typography ml={1}>{getNetwork() === 'mainnet' ? 'Base Mainnet' : 'Base Sepolia'}</Typography>
              </Stack>
            ...
            <Box textAlign={'center'} mt={10}>
              ...
              <Box mt={10}>
                <Button
                  ...
                  style={{ width: 500 }}
                  onClick={() => {
                    window.location.href = '/wallets/base';
                  }}
                >
                  Done
                </Button>
