...
const AvalancheSend = () => {
  ...
  const [coin, setCoin] = useState<string>('AVAX');
  ...
  const getAvax = async () => {
    try {
      ...
      if (response.result) {
        ...
        await getAvaxNonce(response.data.address);
      }
    } catch (e) {
  ...
  const getAvaxGasLimit = async (from: string): Promise<boolean> => {
    try {
  ...
  const getAvaxMaxPriortyFee = async () => {
    try {
  ...
  const getAvaxNonce = async (address: string) => {
    if (address && address != '') {
      try {
  ...
  const checkGasLimit = async (): Promise<boolean> => {
    ...
    return await getAvaxGasLimit(fromAddress);
  };
  ...
  const onClickSignTransaction = async () => {
    ...
    if (displaySign) {
      if (coin === 'AVAX') {
        if (!networkFee || !amount || parseFloat(networkFee) * 2 + parseFloat(amount) > parseFloat(balance['AVAX'])) {
          setSnackSeverity('error');
          setSnackMessage('Insufficient balance or input error');
          setSnackOpen(true);
          return;
  ...
  const init = async (payoutId: any) => {
    await getAvax();
    await getAvaxFeeRate();
    await getAvaxMaxPriortyFee();
          ...
          <>
            ...
            {displaySign && (
              <>
                ...
                <Box mt={4}>
                  <Typography>
                    Miner Fee: {networkFee} AVAX = MaxFee({maxFee}) * Gas({gasLimit})
                  </Typography>
                </Box>
              </>
            )}
          ...
          <>
            <Box textAlign={'center'}>
              <Stack direction={'row'} alignItems={'center'} justifyContent={'center'} mt={4}>
                <Image src={AvalancheSVG} alt="" width={25} height={25} />
                <Typography ml={1}>{getNetwork() === 'mainnet' ? 'Avalanche Mainnet' : 'Avalanche Testnet'}</Typography>
              </Stack>
              ...
              <Box mt={4}>
                ...
                <Stack direction={'row'} alignItems={'baseline'} justifyContent={'center'}>
                  ...
                  <Typography ml={1}>AVAX</Typography>
            ...
            <Box textAlign={'center'} mt={10}>
              ...
              <Box mt={10}>
                <Button
                  ...
                  style={{ width: 500 }}
                  onClick={() => {
                    window.location.href = '/wallets/avalanche';
                  }}
                >
                  Done
                </Button>
