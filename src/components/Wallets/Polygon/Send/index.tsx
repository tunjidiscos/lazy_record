...
const PolygonSend = () => {
  ...
  const [coin, setCoin] = useState<string>('POL');
  ...
  const getPol = async () => {
    try {
      ...
      if (response.result) {
        ...
        await getPolNonce(response.data.address);
      } else {
        setSnackSeverity('error');
        setSnackMessage('The blockchain is currently not bound to any address.');
        setSnackOpen(true);
      }
    } catch (e) {
      setSnackSeverity('error');
  ...
  const getPolGasLimit = async (from: string): Promise<boolean> => {
    try {
  ...
  const getPolMaxPriortyFee = async () => {
    try {
  ...
  const getPolNonce = async (address: string) => {
    if (address && address != '') {
      try {
  ...
  const checkGasLimit = async (): Promise<boolean> => {
    ...
    return await getPolGasLimit(fromAddress);
  };
  ...
  const onClickSignTransaction = async () => {
    ...
    if (displaySign) {
      if (coin === 'POL') {
        if (!networkFee || !amount || parseFloat(networkFee) * 2 + parseFloat(amount) > parseFloat(balance['POL'])) {
          setSnackSeverity('error');
          setSnackMessage('Insufficient balance or input error');
          setSnackOpen(true);
          return;
          ...
          <>
            ...
            <Box mt={4}>
              ...
              <Typography mt={1} color={amountRed ? 'red' : 'none'} fontWeight={'bold'}>
                Your available balance is {balance && balance[coin]} {coin}
              </Typography>
            </Box>
            ...
            {displaySign && (
              <>
                ...
                <Box mt={4}>
                  <Typography>
                    Miner Fee: {networkFee} POL = MaxFee({maxFee}) * Gas({gasLimit})
                  </Typography>
                </Box>
              </>
            )}
          ...
          <>
            <Box textAlign={'center'}>
              <Stack direction={'row'} alignItems={'center'} justifyContent={'center'} mt={4}>
                <Image src={PolygonSVG} alt="" width={25} height={25} />
                <Typography ml={1}>{getNetwork() === 'mainnet' ? 'Polygon Mainnet' : 'Polygon Sepolia'}</Typography>
              </Stack>
              ...
              <Box mt={4}>
                ...
                <Stack direction={'row'} alignItems={'baseline'} justifyContent={'center'}>
                  ...
                  <Typography ml={1}>POL</Typography>
            ...
            <Box textAlign={'center'} mt={10}>
              ...
              <Box mt={10}>
                <Button
                  ...
                  style={{ width: 500 }}
                  onClick={() => {
                    window.location.href = '/wallets/polygon';
                  }}
                >
                  Done
                </Button>
