...
const XrpReceive = () => {
  ...
  const [xrp, setXrp] = useState<string>('');
    ...
    <Box mt={4}>
      <Container maxWidth="sm">
        ...
        <Box mt={4} textAlign={'center'}>
          <Typography>Send only Xrp network assets to this address</Typography>
          <Paper style={{ padding: 80, marginTop: 20 }}>
            <QRCodeSVG
              ...
              height={250}
              imageSettings={{
                src: GetImgSrcByCrypto(COINS.XRP),
                width: 30,
                height: 40,
                excavate: false,
              }}
            ...
            <Box mt={4}>
              <Stack direction="row" alignItems="center" justifyContent="center">
                <Typography mr={1}>{xrp}</Typography>
                <IconButton
                  onClick={async () => {
                    await navigator.clipboard.writeText(xrp);
